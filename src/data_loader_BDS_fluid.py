...
def _den_list_to_blob(ims):
    ...
    max_shape = np.array([im.shape for im in ims]).max(axis=0)
    num_images = len(ims)
    blob = np.zeros((1, 1, max_shape[0], max_shape[1]),
                    dtype=np.float32)
...
class ImageDataLoader():
    def __init__(self, root_path, batch_size = 4, training = True, testing = False, shuffle=True, gt_downsample=False, pre_load=False):
        ...
        self.img_npy_path = self.root_path + "/images/baidu_star_2018_train_stage1/baidu_star_2018/image_3_resize_npy/"
        ...
        if self.training:
            ...
            for iter_id in range(iters):
                ...
                for batch_id in range(self.test_batch_size):
                    ...
                    current_test_img = cv2.imread(image_name, 1)
                    current_test_img = cv2.resize(current_test_img, (1024, 512, 3))
