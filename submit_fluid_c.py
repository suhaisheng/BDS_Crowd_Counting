...
import numpy
from src.data_loader_BDS_fluid_Stage2 import ImageDataLoader  #original version
...
data_loader_test=ImageDataLoader(root_path, training = False, testing=True, batch_size = 1, shuffle=True, gt_downsample=True, pre_load=False)
...
model_name = '_iterative_crowd_counting_dot_bbox_countloss'
...
tag = '_iterative_crowd_counting_dot_bbox_countloss'
# block
...
for blob in data_loader_test:
    if batch_id % 100 == 0:
        print('processing model epoch: %05d  batch_id: %05d'%(epoch, batch_id))        
    val_img = blob['data']
    ...
    et_count = int(round(np.sum(den_es_2)))
    idxs.append(batch_id)
    counts.append(et_count)
    batch_id +=1  
    # idx += 1
...
dataframe = pd.DataFrame({'id':idxs,'predicted':counts})
dataframe.to_csv("submit/result_%d_%s.csv"%(epoch, tag),index=False,sep=',')
